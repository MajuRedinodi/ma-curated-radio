...
| Degrees of separation [3] | How far a session may travel from the artist that started it. Ignored by Artist radio and Discovery. Still applies when seeding from the song, and note what it measures: hops through the *artist* graph. A pick admits the whole crowd at one hop, so it never bites on the first batch, but later refills reseed from an artist already a hop or two out, and on a tight setting the fence will start refusing new names from hour three onward. Raise it if late evenings feel narrow. |
...
| Replay when weaker by [55%] | How far a station may fall below the batch it opened with before records it has already played are allowed back in. Measured against the session's own start, so it means the same in country as in rock where absolute listener counts never do, and judged on the batch about to be queued rather than the one just played. Once past it, up to three already-played records are let back through the repeat window, biggest first, and one cannot return a second time until everything within the same percentage of the best has had its turn. Read from the *previous* batch's measurement, so the correction lands on the very next hour rather than the one after. Refills only. Set by ear across one long session: 46% below the opening batch still sounded right, 53% sounded slightly weak. Set from its own entity. |
...
**The year** is when the record was made, and **the chart placing** where
one was found, both from Wikipedia. A line without a year is a record
nothing has told us about rather than a modern one, and a line without a
placing is a record Wikipedia does not give one for, which is **not** the
same as one that missed the charts: eleven of sixteen canonical records
have no placing on Wikipedia at all, "Heartbreak Hotel" among them. So
the placing is collected and shown, and nothing is ever rejected for
lacking one. See
[docs/chart-probe-2026-09-17.md](docs/chart-probe-2026-09-17.md). **Covers are deliberately left undated.** A cover shares its title
